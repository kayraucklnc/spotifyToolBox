...
const ForceDirectedGraph = ({musicData}) => {
    ...
    useEffect(() => {
        ...
        const simulation = d3.forceSimulation(nodes)
            ...
            .force("charge", d3.forceManyBody().strength(-2000)) // Adjust the charge strength
        ...
        const circles = node.append("circle")
            .attr("r", d => mapValue(d.followers, min, max, 45, 150))
            .attr("fill", d => color(d.group));

        const textLabels = node.append("text")
            .text(d => d.id)
            .attr("dy", 0) // Adjust the text position relative to the circle
    ...
    return <svg ref={svgRef} width="1250px" height="1250px" viewBox={`0 0 ${1250} ${1250}`}/>;
};
