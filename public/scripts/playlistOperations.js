...
function processTracks(tracks) {
  return tracks.map((track) => ({
    ...
    duration: `${Math.floor(track.track.duration_ms / 1000 / 60)}:${Math.floor(
...
async function addCheckedSongsToBasket() {
  ...
  for (let i = 0; i < checkedSongs.length; i++) {
    ...
    const response = await fetch(
      `https://api.spotify.com/v1/tracks/${checkedSongs[i]}`,
      {
        headers: {
          Authorization: `Bearer ${localAccessToken}`,
        },
...
async function removeDuplicates(request) {
  ...
  let duplicateTrackIDs = playlistTrackIDs.filter(
    (item, index) => playlistTrackIDs.indexOf(item) !== index
  );
  let uniqueSet = new Set(duplicateTrackIDs);
  duplicateTrackIDs = Array.from(uniqueSet);
  console.log(duplicateTrackIDs);

  let trackUris = duplicateTrackIDs.map(
